import cgi
'''增加以下两个函数'''
def action_class2dict(action):
    if action == None:
        return None
    else:
        return {'title':action.title, 'type':action.type, 'nextsteps':action.nextsteps}

def actions_convert(actions):
    actions_converted=[]
    for action in actions:
        actions_converted.append(action_class2dict(action))
    return actions_converted
    

def step_class2dict(step):
    if step == None:
        return None
    else:
        return {'name':step.name, 'title':step.title, 'actions':actions_convert(step.values())}

def get_related_step(task):
    '''related_step = task.relatedStep改为下一行'''
    related_step = step_class2dict(task.relatedStep)
    if step_with_children and related_step is step_with_children:
        for child_step in child_steps:
            '''if task.title == child_step.title:改为下一行'''
...
def set_potential_step_names(step):
    ...
    '''for action in step.values():改为下一行'''
    for action in step['actions']:
        '''if action.type == 'error': continue改为下一行'''
        ...
        for next_step_name in action.nextsteps:
            ...
            '''next_step = step_container.get(next_step_name)改为下一行'''
            next_step = step_class2dict(step_container.get(next_step_name))
            if next_step is None:
                '''next_step = child_step_container.get(next_step_name)改为下一行'''
                next_step = step_class2dict(child_step_container.get(next_step_name))
            if next_step is None:
                raise Exception(u"无法根据下一步骤的名字找到步骤对象: %s" % next_step_name)
            set_potential_step_names(next_step)
'''增加下两个函数'''
def ChildAction(title, type, next_steps):
    action={'title':title, 'type':type, 'nextsteps':next_steps}
    return action

def ChildStep(name, title, actions, parent_step_name, real_next_step=''):
    real_actions = []
    for action in actions:
        next_steps = []
        for nextstep in action['nextsteps']:
            if nextstep == parent_step_name:
                if not real_next_step: continue
                else: nextstep = real_next_step
            next_steps.append(nextstep)
        real_actions.append(ChildAction(action['title'], action['type'], next_steps))
    step={'name':name, 'title':title, 'actions':actions, 'parent_step_name':parent_step_name, 'real_actions':real_actions}
    return step
...
def gen_child_steps(step_names, parent_step, next_step_name):
    ...
    parent_step_name = parent_step.name改为下两行'''
    actions = parent_step['actions']
...
def get_child_step_names(datamanager):
    ...
    steps_table = IFieldStorage(datamanager).get('steps', [])
...
def get_next_step_name(steps, index):
    ...
    try:
        '''return steps[index + 1].name改为下一行'''
...
def get_child_steps_info(datamanager, steps):
    ...
    '''step_names = [s.name for s in steps]替换成下一行'''
    ...
    if child_step_names and 'review' in step_names:
        ...
        child_steps_list, child_steps_dict = gen_child_steps(
                child_step_names, steps[index], next_step_name)
...
'''steps = step_container.values()替换成下三行'''
steps=[]
for step in step_container.values():
    steps.append(step_class2dict(step))
# 得到已完成的任务和当前任务
...
if completed_tasks:
    '''init = get_related_step(completed_tasks[0]).name改为下一行'''
    init = init = get_related_step(completed_tasks[0])['name']
...
for n, task in enumerate(completed_tasks):
    ...
    for next_step in next_steps:
        ...
        draw_arrow(related_step.name, next_step_name, translate_task_finish_action(task))改为下两行'''
        next_step_name = next_step['name'] if next_step is not None else 'x__end_flow_node'
...
for step in steps:
    ...
    '''elif step.name not in potential_step_names:改为下一行'''
    ...
    '''draw_box(step.name, '%s (%s)' % (step.i18n.get_text('title') or step.title, ','.join(pid_names)), fillcolor)改为下一行'''
    ...
    for action in step[actions]:
        ...
        for next_step in action.nextsteps:
            ...
            draw_arrow(step.name, next_step,
                str(count) + '.' + (action.i18n.get_text('title') or action.title))改为下十行'''
        if not action['nextsteps']:
            draw_arrow(step.name, 'x__end_flow_node',
                str(count) + '.' + (action.i18n.get_text('title') or action['title']))
            end_box_drawed = True
